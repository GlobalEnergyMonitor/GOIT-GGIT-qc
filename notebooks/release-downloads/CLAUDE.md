# release-downloads (formerly goit-ggit-data-requests)
...
- The only active notebook here is
  `convert-ggit-goit-to-tracker-release-downloads-CURRENT.ipynb`.
...
- Fuel buckets and status lists come from the `gem-tracker-constants` package
  (editable install at `../gem-tracker-constants` relative to the repo root,
  pinned by tag in the notebook's first cell). Never re-declare fuel lists
  inline — change the package and bump the tag instead. The QC summary sheets
  in this repo filter on the same buckets so release totals match QC totals.
